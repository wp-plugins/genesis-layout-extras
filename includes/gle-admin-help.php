...
function ddw_genesis_layout_extras_help_tabs() {
	...
	$gle_help_usage =
		'<h3>' . __( 'Plugin: Genesis Layout Extras', 'genesis-layout-extras' ) . ' <small>v' . esc_attr( ddw_gle_plugin_get_data( 'Version' ) ) . '</small></h3>' .
	...
	if ( ( function_exists( 'minimum_portfolio_post_type' ) || function_exists( 'executive_portfolio_post_type' ) ) && post_type_exists( 'portfolio' ) ) {
		...
		$screen->add_help_tab(array(
			...
			'callback' => 'ddw_gle_admin_help_studiopress',
		) );
	...
	if ( ( CHILD_THEME_NAME == 'Megalithe' || CHILD_THEME_NAME == 'Engrave Theme' || CHILD_THEME_NAME == 'Vanilla' ) && post_type_exists( 'portfolio' ) ) {

		$screen->add_help_tab(array(
			'id' => 'gle-child-theme-support',
			'title' => __( 'Child Theme Support', 'genesis-layout-extras' ),
			'callback' => 'ddw_gle_admin_help_zigzagpress',
		) );
